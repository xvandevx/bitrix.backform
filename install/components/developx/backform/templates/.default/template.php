...
<? if (empty($_REQUEST["AJAX_CALL"]) && $_REQUEST["AJAX_CALL"] != 'Y') { ?>
    <? if ($arParams['GOOGLE_CAPTCHA'] == 'Y') { ?>
        <script src='//www.google.com/recaptcha/api.js?render=<?= $arParams['CAPTCHA_KEY'] ?>'></script>
    <? } ?>
...
<? if ($_REQUEST["AJAX_CALL"] == 'Y') { ?>
    <? if (empty($arResult["RESULT"]) && $arParams['GOOGLE_CAPTCHA'] == 'Y') { ?>
        <script>
            DevelopxBackform_.resetCaptcha();
        </script>
    <? } ?>
<? } else { ?>
    <script>
        $(document).ready(function ($) {
            DevelopxBackform_ = new DevelopxBackform('<?=$arParams['GOOGLE_CAPTCHA']?>', '<?=$arParams['CAPTCHA_KEY']?>');
        });
    </script>
<? } ?>